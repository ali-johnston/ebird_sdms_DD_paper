...
# set species to analyse
all_species <- c("Wood Thrush", "Chuck-will's-widow")


for(i_species in c(1:length(all_species))){

    set.seed(1)

    # set parameters for analysis
    species <- all_species[i_species] #species
    ...
    date <- Sys.Date()
    run_name <- str_glue("RF_sample_{sample_regime}_{sp_code}_{date}")
